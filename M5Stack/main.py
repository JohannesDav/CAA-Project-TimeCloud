...
#Display elements
labelName = M5Label('Welcome, Johannes!', x=22, y=65, color=0x000, font=FONT_MONT_18, parent=None)
labelId = M5Label('UserID: 198-252-26-194', x=22, y=33, color=0x000, font=FONT_MONT_18, parent=None)
labelInstruction = M5Label('Please select your project:', x=22, y=96, color=0x000, font=FONT_MONT_18, parent=None)
touch_buttonNext = M5Btn(text='Next', x=22, y=177, w=110, h=50, bg_c=0xFFFFFF, text_c=0x000000, font=FONT_MONT_18, parent=None)
touch_buttonConfirm = M5Btn(text='Confirm', x=186, y=177, w=110, h=50, bg_c=0xFFFFFF, text_c=0x000000, font=FONT_MONT_18, parent=None)
labelProject = M5Label('Semester Project', x=22, y=132, color=0x000, font=FONT_MONT_22, parent=None)
labelEventInfo = M5Label('Work session start !', x=22, y=128, color=0x000, font=FONT_MONT_26, parent=None)
...
def getUserInfo(user_id):
    print('Sending getUser request')
    url = 'https://europe-west6-cloud-and-advanced-analytics.cloudfunctions.net/GetInfo'
    ...
    if response.status_code == 200:
        ...
        print('Request failed. Error:', response.content)
        return None

def logEvent(user_id, project_id):
    print('Sending logEvent request')
    url = 'https://europe-west6-cloud-and-advanced-analytics.cloudfunctions.net/LogEvent'
    headers = {'Content-Type': 'application/json'}
    ...
    if response.status_code == 200:
        ...
        print('Failed to log event. Error:', response.content)
        return None
        
def setInitialDisplay():
  labelInstruction.set_text('Please scan your badge')

  labelName.set_hidden(True)
  labelId.set_hidden(True)
  labelInstruction.set_hidden(False)
  touch_buttonNext.set_hidden(True)
  touch_buttonConfirm.set_hidden(True)
  labelProject.set_hidden(True)
  labelEventInfo.set_hidden(True)

setInitialDisplay()

while True:
  time.sleep_ms(20)
  # SENSOR INIT, has to be run each loop, since set_text() and set_hidden() break the connection somehow?
  pn532 = PN532(spi_dev, cs, debug=False)
  ...
  # Read card
  cardID = read_nfc(pn532, 3000)
  
  if cardID is not None:
    labelId.set_text("UserID: " + cardID)
    labelId.set_hidden(False)
    labelInstruction.set_hidden(True)
    
    userInfoResp = getUserInfo(cardID)
    print(userInfoResp)
    
    if userInfoResp is not None: # the user exists
      labelName.set_text("Welcome, " + userInfoResp["name"])
      labelName.set_hidden(False)
      
      if userInfoResp["working"]: # the user is currently working, stop the session
        selectedProjectID = "default-stop"
      else: # the user is not currently working
        userInfoResp["projects"].append({"id": "default-start", "name": "No Project"}) #add the default project
        if len(userInfoResp["projects"]) > 1: # the user is assigned to (a) project(s), select one
          labelInstruction.set_text("Please select your project:")
          labelInstruction.set_hidden(False)
          
          selectedIndex = 0
          touch_buttonNext.set_hidden(False)
          touch_buttonConfirm.set_hidden(False)
          labelProject.set_hidden(False)
          prevNext = False
          pressedNext = False
          while not touch_buttonConfirm.get_state():
            stateNext = touch_buttonNext.get_state()
            if stateNext != prevNext:
              prevNext = stateNext
              pressedNext = stateNext
            if pressedNext:
              selectedIndex += 1
              if selectedIndex == len(userInfoResp["projects"]):
                selectedIndex = 0
            pressedNext = False
            labelProject.set_text(userInfoResp["projects"][selectedIndex]["name"])
            time.sleep_ms(10)
            
        else: # the user is assigned to no projects, the default project is selected
          selectedIndex = 0
        
        selectedProjectID = userInfoResp["projects"][selectedIndex]["id"]
      ...
      labelEventInfo.set_text("Sending...")
      labelEventInfo.set_hidden(False)
      
      # start a session with the selected project ID
      logResp = logEvent(cardID, selectedProjectID)
      
      if logResp is not None:
        if userInfoResp["working"]:
          labelEventInfo.set_text("Session stopped !")
        else:
          labelEventInfo.set_text("Session started !")
      else:
        labelEventInfo.set_text("Network error")

    else: #the user is not registered
      labelName.set_text("Please register badge")
      labelInstruction.set_text("or connect to wifi")
      labelName.set_hidden(False)
      labelInstruction.set_hidden(False)
